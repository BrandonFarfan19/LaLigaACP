...
## 2026-09-18 — C-03 · La cabecera de la ficha de plantilla deja de desbordar (cambio posterior al plan)

- **Pedido del usuario:** arreglar que la cabecera de `/plantilla/:id` **desbordara a 320 px**. Salió a la luz al cargar sus 15 equipos reales; es el tercer cambio posterior al plan, cerrado en T-23, así que no se marca nada en [docs/plan-polla.md](docs/plan-polla.md).
- **El fallo:** a **305 px útiles** (320 con la barra de desplazamiento clásica) desbordaban **FINZULIANAS 9 px, LOS IMPARABLES 8 y LAS GALACTICAS DEL MASTER 1**, y otros tres se comían el relleno. La causa son tres cosas juntas: `.identity` es un flex **sin salto** con **tres** hijos (escudo, título y el párrafo de competición), y un elemento flex **no baja de su palabra más larga** mientras su `min-width` sea `auto`.
- **El cambio**, todo en `src/pages/Plantilla.module.css`: **por debajo de 36rem** la cabecera puede pasar a dos filas, el párrafo de competición toma su propio renglón y el título puede encogerse y partirse (`flex-wrap: wrap`, `flex: 1 1 0`, `min-width: 0`, `overflow-wrap: anywhere`, y `flex-basis: 100%` en el párrafo); **de 36rem para arriba se repone el piso** y todo vuelve a su valor original. Prueba nueva `src/pages/plantilla-header.test.ts`, 6 casos que leen la hoja del disco, con uno que exige expresamente que el piso se reponga arriba del tope. En `docs/pendientes.md` se anotaron el límite conocido y **tres defectos preexistentes de T-22** que aparecieron de paso.
  - **Por qué `anywhere` y no `break-word`:** `break-word` parte la palabra **dentro de la caja** pero no baja el `min-content`, así que la cabecera sigue creciendo hasta la palabra entera y empuja la página igual. Lo encontró el ejecutor midiendo, después de que una decisión del coordinador pidiera `break-word`.
  - **El mecanismo, que costó tres intentos entender:** a 768 px **sí** falta espacio —12 px—, porque lo que compite no es el `min-content` del párrafo (90) sino su **`max-content`** (428 px, el de la competición femenina). El piso de cada elemento flex en su propia palabra es lo que decide que **el párrafo absorba esos 12 px y el título quede entero**; quitarle el piso al título lo hace caer por debajo de su palabra. Es **la misma palanca** para las dos cosas: lo único que evita el desborde de una palabra larga es dejar que el título baje de su palabra, y eso es justo lo que parte los nombres reales. Por eso el escape quedó acotado.
- **Decisiones del coordinador (`docs/decisiones.md`): D-029, D-030 y D-031**, tres sobre el mismo punto, y **las dos primeras equivocadas**; se dejaron con el error a la vista en vez de reescribirlas. **D-031** fija el orden de prioridades: (1) que la página nunca desborde en un ancho real, (2) que ningún nombre **realmente cargado** se parta a mitad de palabra, (3) que no cambie el diseño en los anchos grandes y (4), último y sacrificable, la robustez ante un nombre de una sola palabra de 25 o más letras.
- **Archivos:** `src/pages/Plantilla.module.css`, `src/pages/plantilla-header.test.ts` (nuevo), `docs/pendientes.md`, `docs/decisiones.md` (D-029, D-030 y D-031, del coordinador).
- **Verificación (revisión repartida entre dos testers; aprobada tras tres correcciones).** Como en T-23, C-01 y C-02, **solo `tester_liga` usó el navegador** y **solo `tester_liga_2` corrió las suites y el build**. La base de desarrollo tiene los **datos reales del usuario** (15 equipos, 102 jugadores, 134 planteles): **solo se leyó**, y se comprobó al terminar que seguían intactos.
  - **Parte A (`tester_liga`, navegador):** **165 medidas** —15 equipos por 11 anchos, de 305 útiles con barra clásica a 1280—, **cero palabras partidas y cero desbordes**, con la cabecera siempre dentro de los márgenes de 16 px. **FINZULIANAS a 768 px da 264 px de título** (11 letras por 24) **en un solo renglón**, donde en la versión anterior caía a 259 y salía como `FINZULIANA` + `S`. La **línea base** se reconstruyó a mano en el navegador, reponiendo el CSS original de la cabecera, y se comparó equipo por equipo con una firma que **incluye los renglones de texto**: **30 comparaciones a 768 y 1280 px, cero diferencias**. El borde del tope (575 contra 576) se comporta, y el resto de la ficha quedó intacto (cancha, lista con su dorsal, radar con su aviso de datos de muestra y escudo cuadrado con iniciales). De la tabla del límite se reprodujeron **seis filas**, y por debajo de 576 px se probaron 30, 50, 80 y 100 letras sin un solo desborde.
    - **El tester dejó dicho que su observación de la revisión anterior estaba mal fundada:** había medido ancho de caja y fila flex, **no renglones de texto**, y por eso leyó como «misma caja, misma fila» un cambio que en realidad partía el título.
  - **Parte B (`tester_liga_2`):** leyó el diff entero —**150 líneas, todas agregados**, sin tocar otros módulos—; comprobó **declaración por declaración** la reposición arriba del tope; verificó **las doce filas** de la tabla con la aritmética, incluido el salto de **39 letras a 767 px contra 26 a 768** (el título pasa de 16 a 24 px justo en 48rem, y 39 × 16 da lo mismo que 26 × 24); **parseó los archivos `woff` reales** para confirmar que Press Start 2P es monoespaciada de un glifo por em; y escribió **7 mutaciones** que ponen la prueba en rojo. También **corrigió su propia revisión anterior**: había calculado con el `min-content` del párrafo, y el que manda es el `max-content`.
    - **Chequeos:** **307/307** pruebas del front dos veces, `npm run build` sin avisos, y `CLAUDE.md` igual a `AGENTS.md`.
  - **El límite conocido, tal como quedó publicado:** la tabla da el largo de **palabra única** a partir del cual desborda en cada ancho, con **dos columnas** (peor y mejor caso) porque depende del ancho del nombre de la competición, que tampoco baja de su palabra más larga (`masculino` 104 px, `femenino` 90, `voleibol` 86). **Manda la columna de fútbol masculino.** El peor caso de todos es **768 px con 26 letras**, y **por debajo de 576 px no desborda nunca**, con ningún largo. Los 15 equipos de hoy están lejísimos: la palabra más larga es `FINZULIANAS`, 11 letras. **La primera versión de esa tabla publicaba el mejor caso disfrazado de límite**; lo encontró `tester_liga_2` y el ejecutor la rehízo al peor caso.
  - **Al cerrar** se comprobó que el único archivo tocado después de la aprobación fue `docs/pendientes.md`, y que el CSS del árbol **reproduce exactamente** las medidas aprobadas (FINZULIANAS en 264 px y un renglón a 768, y las seis filas del límite).
- **Observaciones (no bloquean):**
  - **`justify-content: center` quedó en `.identity` fuera de la media query** y no se repone arriba del tope. Se midió **inerte** a 768 y 1280 px, porque ahí las filas van llenas y no hay sobrante que alinear, y quedó documentado con esa medición en el propio CSS.
  - **La prueba fija el contrato del CSS, no el no-desborde**, porque jsdom no hace layout; el no-desborde está medido en el navegador. Se rompe con tres reescrituras equivalentes (el selector sin espacios, otra caja de mayúsculas, `576px` en vez de `36rem`): falsa alarma ruidosa, nunca un defecto sin detectar.
  - **La tabla del límite está medida sin barra de desplazamiento clásica.** Con una barra que ocupa lugar (15 px), el largo tolerado baja alrededor de una letra en los anchos donde el título mide 16 px (a 576 px, 27 pasa a 26) y en algún borde de los grandes (1280, de 48 a 47). En el peor caso, 768 px, no cambia.
  - **Los tres defectos preexistentes de T-22** anotados en `docs/pendientes.md` (dos clases que `Plantilla.tsx` pide y no existen, y `.crest` que ya no usa nadie) **se verificaron ciertos y no se corrigieron en C-03**: el pedido era arreglar el desborde sin mover nada más de la página.
  - **El hallazgo de la cancha de 9 lugares no es parte de este cambio.** `src/lib/squad-layout.ts` coloca como mucho 9 jugadores y 4 de los 15 equipos reales tienen 10, así que uno no aparece en pantalla. Lo encontró `tester_liga` al revisar C-03; es anterior y queda para decidir aparte.

## 2026-09-23 — D-03 · Artefactos de despliegue en producción (cambio posterior al plan)

- **Pedido del usuario:** poder desplegar el proyecto en su propio servidor Linux, con nginx en el host y Docker para la base y el backend. Es el cuarto cambio posterior al plan, cerrado en T-23, así que no se marca nada en [docs/plan-polla.md](docs/plan-polla.md).
- **El punto de partida:** el repo **no tenía nada de producción**. `compose.yaml` es de desarrollo de punta a punta: `NODE_ENV: development` escrito literal, el código montado desde el host (`./server:/app`), `CMD` con `nodemon` y, lo más grave para un servidor con IP pública, **`3306` y `3001` publicados en todas las interfaces** — MySQL abierto a internet y el backend accesible saltándose nginx, sin TLS y sin el límite de tamaño de subida.
- **Lo que se agregó:**
  - **`server/Dockerfile.prod`**, multi-etapa: una etapa compila con las devDependencies (`tsc -b`) y la final instala solo producción (`npm ci --omit=dev`, sin `tsx`), corre como usuario `node` y arranca `node dist/index.js` en forma exec, para que reciba el `SIGTERM` que `src/index.ts` ya sabe atender.
  - **`compose.prod.yaml`**, con la base **sin publicar** y el backend **solo en `127.0.0.1`**, nombre de proyecto propio, `DEV_SEED_DATABASE` forzada a vacío y el volumen de imágenes aparte.
  - **`deploy/nginx/la-liga-acp.conf.example`** (el sitio, con marcadores para dominio, certificado, repo y puerto) y **`deploy/nginx/spa-routes.conf`**, generado por el nuevo **`vite-plugins/nginx-spa-routes.ts`**.
  - **La sección de despliegue del README**, con requisitos previos, los 8 pasos en el orden real, el recuadro de las contraseñas de MySQL, el certificado antes de nginx, y los apartados «Qué está probado y qué no», «Por qué está armado así» y «Cuidado con esto».
- **Tres hallazgos del ejecutor que no estaban en el encargo:**
  - **La imagen necesita `package.json`**, no solo `dist/`: `services/health.service.ts` lee la versión con `createRequire`, que desde `dist/services/` resuelve a `/app/package.json`. Sin ese archivo, `GET /health` falla.
  - **`TRUST_PROXY` va en `1`, no en `loopback`.** Aunque nginx llegue a `127.0.0.1:3001`, el puerto se publica con NAT y el contenedor ve como origen la puerta de enlace del bridge de Docker (`172.x.x.x`): con `loopback` el backend ignoraría `X-Forwarded-For` y **todos los clientes compartirían los límites de intentos**.
  - **`compose.prod.yaml` tiene que ser un archivo APARTE, no un override.** Compose fusiona las listas de `volumes` **agregando, nunca quitando**, así que un override no puede sacar el bind mount `./server:/app` ni el volumen anónimo de `node_modules`; medido con `docker compose -f compose.yaml -f override.yaml config`. En producción eso taparía la imagen compilada con el código del host.
- **La configuración de nginx de la SPA se genera, igual que `_redirects`:** `vite-plugins/nginx-spa-routes.ts` la escribe desde `SPA_ROUTES` en cada build. **21 rutas = 42 URLs en 39 bloques** (36 `location =` exactos y 3 expresiones regulares de un solo segmento), **sin comodín `/*`**, y no puede desincronizarse porque se reescribe sola. Se escribe **fuera de `dist/`**, que es la raíz web. A diferencia de `_redirects` **sí se versiona**, con su motivo escrito: si el `include` falta, nginx **no arranca**, así que el archivo tiene que existir apenas se clona.
- **La trampa de nginx que casi se cuela:** una cabecera puesta con `add_header` **no se hereda** en un bloque que tenga la suya. Como la primera versión ponía el `Cache-Control` dentro de cada bloque de la SPA, `Strict-Transport-Security` se habría perdido **en los 39 bloques**, justo en las respuestas más pedidas. Se resolvió con un `map` y las cabeceras una sola vez en el `server`; el bloque `/api/` tiene las suyas a propósito, para no pisar el `Cache-Control` del backend, y eso quedó escrito como regla para quien edite el sitio.
- **Archivos:** `compose.prod.yaml` (nuevo), `server/Dockerfile.prod` (nuevo), `deploy/nginx/la-liga-acp.conf.example` (nuevo), `deploy/nginx/spa-routes.conf` (nuevo, generado), `vite-plugins/nginx-spa-routes.ts` (nuevo), `vite.config.ts`, `README.md`, `server/README.md`, `.env.example`, `.gitignore`, `CLAUDE.md` y `AGENTS.md`.
- **Verificación (revisión repartida entre dos testers; aprobada tras dos rondas).** `tester_liga` revisó la documentación siguiendo el README de punta a punta como alguien recién llegado a un Linux limpio, sin navegador y sin Docker; `tester_liga_2` revisó los artefactos.
  - **Primera ronda, cuatro fallos de documentación, todos corregidos:** las contraseñas de MySQL se graban al crear el volumen y el README no lo decía ni ordenaba los pasos en consecuencia (ahora está en el README, en `.env.example`, en `compose.prod.yaml` y con el `.env` terminado antes de levantar nada, más las dos salidas: `ALTER USER` conservando los datos o borrar el volumen); faltaba la tabla de requisitos previos con versiones y comandos de comprobación; el certificado no tenía sección propia y el caso de `certbot` chocaba con un huevo y la gallina; y `server/README.md` contradecía al README diciendo que el administrador se crea con `npm run admin:create`, que en la imagen de producción **no existe**.
  - **Segunda ronda, diez observaciones, todas aplicadas:** el aviso y los `sed` de los marcadores (cada uno aparece dos veces y olvidar el segundo `DOMINIO.EJEMPLO` falla en silencio), el `ALTER USER` de root ampliado a las **dos** cuentas (`root@localhost` y root con comodín, que es la que usa la suite del backend para conectarse por TCP), la recuperación con `--init-file` como paso previo a borrar el volumen, los dos huecos del caso `certbot` (el `server_name _;` del sitio por defecto y el `conflicting server name` que `nginx -t` solo avisa), el `restart` que no relee el `.env` cambiado por `up -d server`, el alcance real de lo probado, la decisión sobre versionar `spa-routes.conf`, el título de la sección de desarrollo, las rutas propias de Debian y Ubuntu, y la contraseña de MySQL pedida con `-p` en vez de escrita en el comando.
  - **Comprobado en la re-revisión:** los tres `sed` aplicados sobre una copia real del sitio reemplazan **las seis apariciones** y el `grep` de control imprime **0**; `spa-routes.conf` y `dist/_redirects` cubren **exactamente las mismas 42 URLs** (comparadas una por una, cero diferencias en los dos sentidos); las expresiones regulares aceptan `/plantilla/42` y `/plantilla/42/` y rechazan `/plantilla/a/b`, `/plantilla` y `/plantilla/`; los cinco `curl` del paso 8 dan lo prometido contra la configuración real; y no quedó **ningún rastro** de la arquitectura descartada (nginx dentro de Docker).
  - **`tester_liga_2` verificó la herencia de cabeceras con un simulador** que expande el `include` y aplica la precedencia real de nginx, en vez de leer el archivo y razonarla.
- **Observaciones (no bloquean):**
  - **El respaldo de «Cuidado con esto» expande `$MYSQL_ROOT_PASSWORD` en la terminal del host**, donde normalmente no está definida, y deja la contraseña en la línea de comandos. La sección de desarrollo ya lo hace bien (`sh -c` con comillas simples, para que la variable se expanda **dentro** del contenedor); conviene igualarlo y agregar `-T` por la redirección.
  - **«Cada marcador aparece dos veces» es exacto para las directivas activas**; contando la cabecera de comentarios del archivo hay tres de cada uno. No cambia nada porque los `sed` llevan `g` y por eso el `grep` de control da 0.

## 2026-09-23 — D-04 · Migración de los datos reales a producción (cambio posterior al plan)

- **Pedido del usuario:** llevar a la base de producción los datos que ya cargó en desarrollo (3 deportes, 3 competiciones, 15 equipos, 102 jugadores y 134 planteles, de las cargas D-01 y D-02), porque la base de producción nace vacía: al crear el volumen solo corren `db/init/01-schema.sql` y `db/init/02-catalogos.sql`.
- **El cambio:** `npm run db:export-real` (`db/export-datos-reales.mjs`) genera `db/datos-reales.sql` desde la base de desarrollo, y el README explica los tres pasos: generar, copiar con `scp` y cargar dentro del contenedor de producción entrando como **root de MySQL** (el usuario de la aplicación solo tiene permisos sobre su base).
- **Qué viaja y qué no, y por qué:**
  - Viajan **solo cinco tablas** del Módulo Informativo —`deporte`, `competicion`, `equipo`, `jugador` y `plantel`— en el orden de las claves foráneas, **conservando los ids**: las URLs de la app son `/plantilla/42`, o sea que el id es visible y estable, y `plantel` referencia equipos y jugadores por id.
  - **No viajan los 9 catálogos**: los carga `02-catalogos.sql` al crear el volumen, y repetirlos rompe por `codigo` único.
  - **No viajan `usuario`, `sesion`, `auditoria` ni nada de Polla.** En la base de desarrollo hay **1 usuario, 1 sesión y 7 registros de auditoría**, y se quedan a propósito: llevar hashes de contraseña y sesiones de una máquina de desarrollo a un servidor real es justo lo que no hay que hacer. El administrador de producción se crea allá, con el comando del paso 7.
  - **No se olvidó nada:** se comprobó que `partido`, `partido_equipo`, `gol` y `multimedia_partido` están en **0** filas, así que no hay partidos que llevar (se cargan desde el panel).
  - El exportador **solo lee**: lo único que corre contra MySQL son `mysqldump` y `SELECT COUNT(*)`.
- **Falla limpio en vez de ser idempotente, a propósito.** En lugar de `INSERT IGNORE` o `REPLACE`, el volcado lleva una barrera que **aborta si las cinco tablas no están vacías**, todo dentro de una transacción que solo llega al `COMMIT` si la verificación de conteos da bien. Es una carga inicial única sobre una base nueva: si ya hay algo, lo correcto es detenerse y mirar por qué, no mezclar en silencio. Los dos abortos se ven como el error de una tabla inexistente con nombre hablado (`ABORTADO_la_base_ya_tiene_datos_informativos`, `VERIFICACION_FALLIDA_los_conteos_no_coinciden`), y el README trae la tabla de qué pasó y qué hacer en cada caso.
- **El volcado no se versiona** (`.gitignore`): lleva los nombres de **102 personas reales** y además es un artefacto derivado de la base de desarrollo, no una fuente. Se genera y se copia con `scp`.
- **Archivos:** `db/export-datos-reales.mjs` (nuevo), `package.json` (script `db:export-real`), `.gitignore`, `README.md` (sección «Llevar los datos reales a producción»). `db/datos-reales.sql` se genera y no se versiona.
- **Verificación:** carga real en una base desechable, no en la de desarrollo, que solo se leyó.
  - Cargó **3, 3, 15, 102 y 134** filas, con `Verificación OK`.
  - **Los ids se conservan:** los equipos entran como 49 a 63, tal como están en desarrollo.
  - **Los acentos se leyeron de vuelta desde la base** (`AQUÍ SE COBRA FC` y demás), no solo desde el archivo.
  - **Una segunda corrida aborta sin duplicar ni una fila.**
  - **Un volcado adulterado se detecta y DESHACE toda la carga:** la verificación de conteos falla, la transacción no llega al `COMMIT` y la base queda vacía.
- **Observaciones (no bloquean):**
  - **La salida de emergencia del paso 3 es dudosa.** Si `MYSQL_ROOT_PASSWORD` no está en el entorno, el README ofrece dos caminos: `set -a; . ./.env; set +a`, que funciona, y `-p` sin valor «para que la pida», que en ese comando concreto es improbable que funcione — va con `exec -T` (sin terminal en el contenedor) y con la entrada estándar tomada por el propio volcado. Conviene dejar documentado solo el primero.
  - El nombre de la base va escrito (`la_liga_acp`) en el comando de carga: quien cambie `MYSQL_DATABASE` tiene que cambiarlo ahí también.

## 2026-09-23 — D-05 · Prueba de humo de la pila de producción (cambio posterior al plan)

- **Qué es:** la **ejecución real** de todo lo que D-03 y D-04 solo dejaban escrito. Hasta acá los artefactos se habían revisado leyéndolos, con Docker apagado; en D-05 se levantó la pila entera contra Docker real y se midió.
- **Lo que se midió:**
  - **La imagen se construye en 15 s y no compila nada nativo:** no aparecen `node-gyp`, `make`, `g++` ni «building from source» en ninguna línea del build — `npm ci --omit=dev` baja binarios musl ya compilados. Pesa **339 MB** con **117 paquetes**, contra 221 en la etapa de build.
  - **`argon2` y `sharp` se EJECUTARON dentro de la imagen**, no solo se instalaron: hashea y verifica (`$argon2id`) y recodifica a webp. Era el riesgo declarado de usar Alpine (musl).
  - **La imagen lleva lo que dice y nada más:** sin `/app/src`, sin `tsx`, `nodemon`, `typescript` ni `vitest`; con `dist/index.js`, `dist/cli/create-admin.js` y `/app/package.json`. El proceso corre con **uid 1000** (`node`) y `/data/uploads` le es escribible.
  - **La pila levanta:** `db` *healthy* a los **5 s**, `db/init/` ejecutado con las **24 tablas** y sus catálogos, y `GET /health` **200 con `version 0.1.0`** — que es la prueba en vivo de que `package.json` resuelve desde `dist/services/`, el primer hallazgo de D-03.
  - **La cookie de sesión real:** `__Host-liga_sid` con `Secure`, `HttpOnly` y `SameSite=Strict`.
  - **El administrador se creó con el comando de producción** (`node dist/cli/create-admin.js`, variante `ADMIN_PASSWORD_STDIN`), sin `tsx`, y después `login` 200 y `/admin/participantes` 200.
  - **El volcado de D-04 se cargó con el comando exacto del paso 3** y se verificó también **por la API pública**, no solo contando filas en MySQL.
  - **nginx 1.31.4 aceptó el archivo del proyecto:** `nginx -t` *syntax is ok* y *test is successful*, **sin advertencias**, y los **cinco `curl`** del paso 8 dan lo prometido (portada 200, `/posiciones` 200, `/no-existe` 404, `/api/health` con su sobre, HTTP 301 con `Location` a https).
  - **Las cabeceras se midieron, no se razonaron:** `no-store`, HSTS y `nosniff` en el shell y en las rutas de la SPA; `/assets/` la única inmutable; `/api/health` conserva el `no-store` del backend y `/api/public/deportes` su `max-age=30`, o sea **nginx no pisa ninguno**; y `/api` pelado responde 308. Coinciden con lo que D-03 había razonado sobre el `map` y la herencia de `add_header`.
- **Lo que más vale de esta prueba: el primer `nginx -t` PASÓ Y ERA FALSO.** Git Bash convirtió las rutas de los montajes al estilo de Windows, así que el contenedor de nginx nunca vio el archivo del proyecto y **cargó su configuración de fábrica**; el `test is successful` no decía nada. El tester lo detectó porque el `server_name` respondía `localhost` y el redirect de HTTP devolvía **200 en vez de 301**, repitió todo con `MSYS_NO_PATHCONV=1` y recién entonces midió de verdad. Sin esa sospecha, el informe habría sido un OK sin valor: es la parte de D-05 que conviene recordar.
- **Archivos:** ninguno del producto. D-05 es ejecución y medición; lo que dejó escrito es la sección **«Qué está probado y qué no»** del `README.md`, que reemplazó a la anterior («no se pudo probar acá») y ahora separa lo medido de lo que **sigue sin probarse**: `npm ci` y `npm run build` sobre Linux, `certbot` en sus dos variantes, el firewall y el DNS, y el despliegue sobre el hardware y el dominio reales.
- **Verificación:** `tester_liga` comprobó que esa sección ya no afirma lo que D-05 desmintió (que la imagen nunca se construyó y que nginx nunca pasó por `nginx -t`) y que lo que deja como no probado es solo lo que de verdad sigue sin probarse, sin pasarse de optimista ni quedarse corta.
- **Observaciones:** ninguna que bloquee. La recuperación de la contraseña de MySQL (opción A del recuadro) tampoco se ejecutó nunca; es un procedimiento de emergencia, no parte del despliegue.

...
async function main(): Promise<number> {
	...
	try {
		...
		console.log(`Base ${env.db.database}: ${report.revisados} participante(s) revisado(s).`);
		...
		for (const a of report.adminsConMonedas) {
			console.log(`  ADMIN CON MONEDAS usuario ${a.usuarioId} (${a.email}): saldo ${a.saldo}, ${a.movimientos} movimiento(s)`);
		}
		if (report.ok) {
		...
		console.log(
			`Encontrados: ${report.descuadres.length} descuadre(s), ${report.adminsConMonedas.length} admin(s) con monedas. No se corrigió nada.`,
		);
		return 1;
	} catch (error) {
